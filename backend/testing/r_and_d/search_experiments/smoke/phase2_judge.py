...
async def run_smoke(max_results: int = 8):
    ...
    rule("1. extract_criteria(query)  ->  weighted, content-only criteria")
    ...
    rule(f"2. OpenAlexService.search(query, max_results={max_results})  ->  candidates")
    ...
    rule("3. judge_papers(query, papers)  ->  per-paper 0-3 level + score + summary")
    ...
    rule("4. level distribution  (recall@k_est counts ONLY L3 / Perfect)")
