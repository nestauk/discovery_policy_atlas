...
from _backend import get_llm, get_llm_processor_cls
from config import CONFIG
from reporting.metrics import bucket_0_to_3, relevance_criteria_score
...
def extract_criteria(
    ...
    Frozen instrument: determinism comes from the on-disk cache (extracted once per query),
    not temperature — gpt-5.x is a reasoning model whose API only accepts temperature=1.0.
    Result reused by every arm, the reformulation loop, the report, and the companion judge spec.
    """
    ...
    from langchain_core.prompts import (
        ChatPromptTemplate,
    )  # heavy dep; kept lazy (not via _backend)
    ...
    # arg). Determinism is the cache's job, not this knob's.
    llm = get_llm(CONFIG.models.criteria_model, 1.0).with_structured_output(CriteriaSet)
...
async def judge_papers(
    ...
    *,
    ...
    LLMProcessor = get_llm_processor_cls()
    processor = LLMProcessor(
        ...
        # gpt-5.x reasoning models only accept the default temperature (1.0); LLMProcessor
        # defaults to 0.0, which a strict endpoint would reject. Override explicitly.
        temperature=1.0,
    )
...
def get_cached_scores(query_id: str) -> dict[str, float]:
    """Return {paper_id: continuous weighted-criteria score in [0,1]} for one query.

    Companion to `get_cached_levels`: the Phase-6 arm `judge_fn` sets `Candidate.relevance_score`
    from this so the ranking content blend can use the continuous score rather than the bucketed
    `level/3` (PF sorting.py:512; ranking._relevance_judgement_score). Same underlying cache as
    the levels — judged once, read either way.
    """
    df = _consolidate(RAW_DIR, CRITERIA_DIR, [query_id])
    return dict(zip(df["paper_id"], df["score"]))
