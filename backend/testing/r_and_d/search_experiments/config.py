...
class Budgets:
    ...
    FUTURE WORK (deferred 2026-06-17): the mechanism (BTS, snowball, short-circuit) is
    mode-agnostic — it reads everything here — so two follow-ups are pure config overlays,
    not rewrites: (1) a 3-iteration diligent sweep (PF's diligent default is 3; we start at
    2 below), and (2) a quick/fast mode (PF `config.extra.fast_mode.toml`: quota 250->75,
    dense_top_k 250->100, snowball 200->100, s2_relevance 250->50). See spec §8.
    """
    ...
    # diligent: 2 retrieval iterations (§4.3a). PF's diligent default is 3; we start at 2
    # and plan a 3-iteration sweep later (see class docstring).
    n_search_iterations: int = 2
...
class SnowballWeights:
    ...
    NB: `seed_rel` is the seed's RAW 0-3 judged level (PF uses `relevance_judgement.relevance`,
    not a [0,1] normalisation); these biases are calibrated against that 0-3 scale (snowball.py).
    """
...
class Config:
    ...
    # Politeness: keep the S2 client *below* 1 req/s cumulative (spec §7, §9). Bumped 1.1→1.2
    # after the Phase-8 Arm C run still tripped 429s at 1.1 (FINDINGS 2026-06-26) — trade a little
    # throughput for fewer backoff stalls.
    s2_min_request_interval_s: float = 1.2
    # OpenAlex retry — production's values (max_retries=3, backoff=0.5 ≈ 3.5s of riding). The 06-26 run
    # hardened these to 6/1.0 (~31s) as a PRE-SKIP band-aid: back then a transient 500 that outlived the
    # retry became a fatal RetryError. That's no longer the resilience mechanism — the per-variant
    # catch-and-skip (mirroring prod's gather(return_exceptions=True), references.py:643) tolerates a
    # failed boolean, and the 1 req/s heavy-query gate smooths the burst that caused the transients. So we
    # revert to prod's retry (FINDINGS 2026-06-30): faster (~3.5s vs ~31s per doomed variant) and truer to
    # the Arm A baseline (a long backoff lets A recover queries prod would skip). Applied experiment-side
    # in `_backend.get_openalex_service` (pyalex global); production untouched. `n_search_failures` on the
    # arm result measures any skips, so we can bump these back up if servable queries are being lost.
    openalex_max_retries: int = 3
    openalex_retry_backoff_factor: float = 0.5
    # Status codes pyalex retries (experiment-side). pyalex's default is [429, 500, 503]; we ADD 502/504
    # so transient OpenAlex GATEWAY errors/timeouts get ridden out before the per-variant skip kicks in.
    # FINDINGS 2026-06-30: heavy booleans 504-timeout *transiently* under cluster load (a 51-op subset of
    # q02 504'd once then returned 200 6/6 on retry) — so 504 is retryable, not terminal. Applied in
    # `_backend.get_openalex_service` (pyalex global); production untouched.
    openalex_retry_http_codes: tuple = (429, 500, 502, 503, 504)
    # OpenAlex client-side rate limit for its HEAVY-QUERY governor: any query with >5 boolean operators
    # (OR/AND/NOT) is capped at 1 req/s per client, and heavy booleans intermittently 500 (FINDINGS
    # 2026-06-30: q02 Arm B's 71-operator boolean alternates 500/429, the 429 names the rule). Applied
    # ONLY to >5-operator requests (snowball's ~400 simple calls/iteration stay full-speed) at the pyalex
    # chokepoint via `retrieval._openalex_throttle`, installed from `_backend`; production untouched. 1.1s
    # = the 1 req/s floor + 10% jitter margin; bump toward the S2 value (1.2) if 429s persist (S2 needed
    # exactly that — see s2_min_request_interval_s). The retry above still rides out transient burst-500s.
    openalex_min_request_interval_s: float = 1.1
