...
HIGHLY = (
    2
)  # "Highly Relevant and above" cutoff (levels 2–3) for the parallel relaxed metrics
...
def count_relevant(
    ...
    threshold: int = PERFECT,
) -> int:
    """Count judgements at `level >= threshold`, optionally restricted to a set of ids.

    Default threshold=PERFECT=3 reproduces BENCH count_relevant exactly (levels are 0..3, so
    `>= 3` ≡ `== 3`) — the parity-pinned Perfect metrics are untouched. Pass `threshold=HIGHLY`
    (=2) to count "Highly Relevant and above" for the PARALLEL relaxed metrics; both cutoffs are
    computed side by side from the same judged levels (retrospective, no re-run).
    """
    allowed = set(restrict_to) if restrict_to is not None else None
    return sum(
        1
        for pid, level in judgements.items()
        if level >= threshold and (allowed is None or pid in allowed)
    )
...
def recall_at_k_est(
    ...
    judgements: Judgements,
    pool_perfect_count: int,
    threshold: int = PERFECT,
) -> float:
    """Headline metric: relevant papers (level >= threshold) in the top-k_est judged window ÷ k_est.

    `pool_perfect_count` is the pooled relevant count AT THIS THRESHOLD (§4.6), which sets k_est —
    so the numerator and the k_est denominator share one cutoff. Default threshold=PERFECT is the
    frozen headline; pass HIGHLY (with the matching pool_highly count) for the parallel ≥2 variant.
    Denominator is k_est itself, so recall is capped at 1/factor by construction.
    """
    k = k_est(pool_perfect_count)
    window = _judged_in_order(ranked_ids, judgements)[:k]
    return count_relevant(judgements, restrict_to=window, threshold=threshold) / k
