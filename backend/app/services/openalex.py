...
class OpenAlexService:
    ...
    async def search(
        ...
        if date_from:
            works_query = works_query.filter(from_publication_date=date_from.isoformat())
        ...
        results = []
        for page in works_query.paginate(per_page=min(25, max_results), n_max=max_results):
            results.extend(page)
