...
from app.services.analysis.prompts import (
    ...
    INNER_SETTING_OPTIONS_SYSTEM_PROMPT,
...
class SearchWizardService:
    ...
    async def generate_inner_setting_options(
        ...
        """
        Generate inner setting options for a research question, ordered by commonality.

        Args:
            research_question: The research question to generate options for
            max_options: Maximum number of options to generate (default: 5)
            project_id: Optional project ID for Langfuse tracking
            user_id: Optional user ID for Langfuse tracking

        Returns:
            List of inner setting options ordered from most common to least common
        """
        logger.info("🔍 Generating inner setting options for: '%s'", research_question)
        ...
        try:
            ...
            messages = [
                SystemMessage(content=INNER_SETTING_OPTIONS_SYSTEM_PROMPT),
                HumanMessage(content=f"Research question: {research_question}"),
            ]
            ...
            if langfuse_handler:
                ...
                config["metadata"] = build_langfuse_metadata(
                    tags=[
                        "component:search_wizard",
                        "component:search_wizard.inner_setting_options",
                    ],
                    session_id=session_id,
                    user_id=user_id,
                    project_id=project_id,
                )
                config["run_name"] = "search_wizard.inner_setting_options"

            resp = await llm.ainvoke(messages, config=config)
            content = (resp.content or "").strip()

            if not content:
                logger.warning("❌ No response from AI for inner setting options")
                return []

            # Parse the JSON array
            try:
                inner_setting_options = json.loads(content)
                if not isinstance(inner_setting_options, list):
                    raise ValueError("Expected JSON array")

                # Limit to requested number and ensure they're strings
                inner_setting_options = [
                    str(opt).strip()
                    for opt in inner_setting_options[:max_options]
                    if opt
                ]

                logger.info(
                    "✅ Generated %d inner setting options", len(inner_setting_options)
                )
                return inner_setting_options
            ...
            logger.error(f"Error generating inner setting options: {e}")
            return []
