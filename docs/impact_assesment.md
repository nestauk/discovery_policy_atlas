...
- **Intervention-level context fit**:
  - transferability rating + breakdown (inner setting, population, geography, resource intensity, delivery complexity)
...
2. **Canonical concept creation**
   ...
   - Load **target context** (inner setting / population / geography) from project `search_query`.
...
5. **Impact synthesis (enrichment)**
   - Compute and attach:
    ...
    - transferability rating + breakdown (5 dimensions)
