...
- Agentic loop — adaptive judging (Batched Thompson Sampling), `HighlyRelevantShortcircuit` (≥1 Perfect AND Σ score ≥ 50; +2/Perfect, +1/Highly i.e. level 2), quota 250: `agents/.../dense/relevance_loading_optimization.py`; diligent vs fast config: `conf/config.toml`, `conf/config.extra.fast_mode.toml`
...
- *Multi-query keyword search:* reuse **v2's own production boolean-query generator**
  (`backend/app/services/openalex.py::search_multi_query`, prompt + `BOOLEAN_QUERY_MODEL` from
  `app/services/analysis/prompts.py`) to produce N=5 diverse **boolean** formulations from the
  Step-0 content core; each retrieved at `per_page=200`. This is the *same generator Arm A uses*
  — see the **query-formulation decision (2026-06-21)** at the end of this section. PF's
  "diverse angles, no 'papers about' phrasing" principle still governs the N-way diversity, but
  the formulation backend is v2's, not a fresh PF-derived OpenAlex prompt.
...
- *Query reformulation:* re-invoke **v2's boolean generator** with the top judged-relevant
  papers passed as exemplars/context (consistent with the Step-1 decision below — the
  reformulation idiom matches the initial idiom: boolean, via v2's generator, not a port of PF's
  `_dense_reformulate_prompt_tmpl`). PF's reformulation *strategy* (reformulate from judged
  exemplars) is preserved; only the idiom/backend is v2's. Retrieve again.
...
≥1 Perfect found AND accumulated score ≥ 50 (+2/Perfect, +1/Highly i.e. level 2) — identical
to Arm C (PF: `agents/.../dense/relevance_loading_optimization.py`). *(Replaces the old "no new
Perfect in the last 50 judgments" heuristic — incidental, removed.)* **Scoring fix (2026-06-17):**
earlier drafts said "+1/Somewhat" (level 1); PF's `HighlyRelevantShortcircuit` and `config.py`
accumulate **level ≥ 2** (`score_per_highly=1`), so this is the faithful port. Levels 0–1
contribute 0.
...
score   = w_content × content
        + w_recent  × recency_score(year)        (recent_first / recent_last, by intent)
        + w_central × centrality_score(cited_by_count)  (central_first / central_last, by intent)
```
> **Recency/centrality sigmoids (impl note, 2026-06-17).** The `recency_score` / `centrality_score`
> terms are **ported verbatim from PF `agents/.../common/sorting.py`** (`recent_first_score_sigmoid`
> center_shift=7.0/steepness=0.7, `recent_last` 20.0/0.15; `central_first_score_sigmoid` centred at
> 50 cites). Earlier drafts wrote the shorthand `sigmoid(year − baseline_year)` /
> `sigmoid(log(cited+1))`, but that is **mis-calibrated** (the newest paper would score only 0.5
> and the citation sigmoid is ≥0.5 for every paper), so it is illustrative only — the sigmoid
> *shape* is mechanism, and "this spec wins" (§4.3a) governs the operating point, not the
> mechanism. The terms are **direction-aware**: the Step-0 recency/centrality intent selects the
> first/last variant. `baseline_year` (PF's `year_max`) defaults to the candidate pool's newest
> year (self-relative + offline-deterministic), where PF uses the current calendar year.
...
> **Query-formulation decision (2026-06-21) — chosen, not source-forced.** Arm B formulates its
> keyword leg with **v2's own production boolean generator** (`search_multi_query`), the *same
> generator Arm A uses* — *not* a fresh PF-keyword-prompt-adapted-to-OpenAlex, and *not* a single
> frozen v2 query. The reasoning:
> - **Source-appropriate.** PF's keyword leg is explicitly tuned "for the Semantic Scholar search
>   engine" (`broad_search_by_keyword_prompts.py`) and emits a stripped keyword bag; OpenAlex is
>   boolean-native and v2 already has a tuned boolean generator + multi-query expansion + a query
>   sanitiser. Porting PF's S2 keyword prompt onto OpenAlex would be its own infidelity.
> - **Sharpens A→B.** With Arm A and Arm B sharing the *same* formulation idiom (v2 boolean),
>   A→B isolates the **agentic loop** (multi-query expansion + adaptive judging + snowball +
>   reformulation + rerank) rather than confounding "the machinery helped" with "an S2-tuned
>   formulation mis-fired on OpenAlex."
> - **Machinery, not a single shot.** We keep PF's *formulation strategy* — N=5 diverse
>   formulations + reformulation from judged exemplars — and only swap the *idiom/backend* to
>   v2's boolean generator. Freezing Arm B to one static v2 query (the rejected option) would
>   disable query expansion/reformulation and make A→B test less than the full machinery.
>
> Faithfulness note: this is a deliberate deviation from "verbatim PF port" for Arm B's
> formulation leg (same category as the source-forced differences, but *elective*). The
> "mechanism-identical to Arm C" claim above holds for the *loop*; the *formulation idiom* differs
> by design (Arm C: PF keyword-bag + dense; Arm B: v2 boolean). This rides with the source — see
> the B↔C bundle caveat (§7). Wiring: the OpenAlex `SourceClient.formulate_queries` /
> `.reformulate` (Phase 4) delegate to v2's generator; `broad_search` is unchanged.
...
+2 per Perfect and +1 per Highly (level 2) (`highly_relevant_cap=50`; levels 0–1 contribute 0).
This is Paper Finder's actual stopping rule, **identical to Arm B** (§4.3) — both arms share it
so judged-set sizes are comparable. **Record which condition fired** (budget vs short-circuit) for each query/arm.
...
score   = w_content × content
        ...
        + w_central × centrality_score(cited_by_count)  (central_first / central_last, by intent)
```
`recency_score` / `centrality_score` are the **PF `sorting.py` sigmoids**, direction-aware on the
Step-0 intent — see the Arm B Step-5 impl note (§4.3) for the calibration and the shorthand
correction. `w_*` from the Step-0 intent using the **same weight table as Arm B (§4.3 Step 5)**. Cohere
...
**Checkpoints**
- *After step 3 (core built):* dry-run judging cost/query against estimate (~600–900 judgments/query incl. normalizer pool). Cost is **logged as a finding (§2), not a trigger for code changes** — if it is far off estimate, factor that into **Phase-5 query curation** (which still precedes arm wiring) rather than a mid-build refactor or scope cut. The dry-run is a measurement step, not a gate.
...
- **B↔C source bundle (not a single atomic variable):** with the machinery unified (§4.3),
  ...
  buys you" as a bundle, not "S2 corpus alone." **The query-formulation idiom rides in this
  bundle too** (Arm B: v2 boolean generator; Arm C: PF keyword-bag + dense — the elective
  decision in §4.3): this is deliberate (each source is queried in its native idiom, which is
  *part of* what the source is), but it means B−C is "S2 + dense + influence + abstracts + its
  native formulation," not the source corpus alone. Mitigation: origin tags (§4.7) decompose it
